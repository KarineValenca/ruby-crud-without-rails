...
RSpec.describe TagsController do
    ...
    context "update" do
        it "sucessful update" do
            @updated_tag = TagsController.update(1, "name", "tag test2")
            expect(@updated_tag.id).to eq 1
            expect(@updated_tag.name).to eq "tag test2"
        end

        it "unsucessful update" do
            @updated_tag = TagsController.update(2, "name", "test2")
    ...
    context "delete" do
        it "successful delete" do
            @updated_tag_list = TagsController.delete(1)
            expect(@updated_tag_list).to be_empty
        end

        it "unsuccessful delete" do
            @updated_tag_list = TagsController.delete(2)
            expect(@updated_tag_list).to eq nil
        end
    end
end